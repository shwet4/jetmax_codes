...
class BallTracking:
    ...
    def __init__(self):
        ...
        self.color_x_pid = hiwonder.PID(0.12, 0.001, 0.005)
        self.color_y_pid = hiwonder.PID(0.15, 0.001, 0.0005)
...
def ball_tracking(image):
    ...
    if target_color_range is not None:
        ...
        if circle is not None:
            ...
            vy = 0
            vw = 0 
            if abs(x - ORG_PIXEL_X) > 30: # 左右方向上的PID处理
                state.color_x_pid.update(x - ORG_PIXEL_X)
                vx = state.color_x_pid.output # PID输出
            #print(circle)
            if abs(r - 50) > 10: # 前后方向上的PID处理
                if r - 50 < 0:
                    state.color_y_pid.setKp(1.2)
                else:
                    state.color_y_pid.setKp(0.8)
                state.color_y_pid.update(r - 50)
                vy = -state.color_y_pid.output # PID输出
            # 计算合速度
            vw1 = vy - vx + vw * (1 + 2)
            vw2 = vy + vx - vw * (1 + 2)
            vw3 = vy - vx - vw * (1 + 2)
            ...
            vw1 = 100 if vw1 > 100 else vw1 #限制速度最大最小值
            ...
            en_mot.set_speed(int(vw1), 1)  #驱动底盘电机
...
if __name__ == '__main__':
    ...
    jetmax.set_position((0, -130, 0), 1.5)
    rospy.sleep(1.5)
